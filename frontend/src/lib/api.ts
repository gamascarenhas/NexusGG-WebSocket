...
export async function atualizarUsuario(id: string, nome: string): Promise<Usuario> {
...
export async function buscarConversa(usuarioId1: string, usuarioId2: string): Promise<Mensagem[]> {
  const res = await fetch(`${URL_API}/mensagens/${usuarioId1}/${usuarioId2}`);
  return res.json() as Promise<Mensagem[]>;
}

export async function buscarNaoLidas(usuarioId: string): Promise<Record<string, number>> {
